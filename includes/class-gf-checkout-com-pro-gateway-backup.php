...
class GF_Checkout_Com_Pro_Gateway extends GFPaymentAddOn {
	...
	private function get_payment_form_html( $form, $entry, $feed, $payment_method ) {
		...
		$error_message = '';
		if ( ! empty( $this->payment_page_error ) ) {
	...
	public function process_callback( $feed, $entry, $payment_response ) {
		...
		if ( isset( $payment_response['response_summary'] ) && ! empty( $payment_response['response_summary'] ) ) {
			...
			$response_summary = 'Response code: ' . $payment_response['processing']['partner_response_code'];
		}
	...
	private function checkout_com_process_callback_action( $action ) {
		...
		switch ( $action['type'] ) {
			case 'complete_payment':
				...
				break;
			case 'fail_payment':
				// Check if this transaction ID was already processed (for webhook compatibility)
				$current_transaction_id = rgar( $entry, 'transaction_id' );
				$new_transaction_id = rgar( $action, 'transaction_id' );
				
				if ( $current_transaction_id === $new_transaction_id && rgar( $entry, 'payment_status' ) === 'Failed' ) {
					break;
				}

				error_log( 'Checkout.com Pro: ACTION - Processing fail_payment for entry ' . $entry['id'] . ', Transaction ID: ' . $new_transaction_id );
